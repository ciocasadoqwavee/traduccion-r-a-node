IPCCTier2SOMmodel <- function(SiteDataFile="NAME", WthFile="NAME", ParameterFile="NAME", init.active=100,
                              init.slow=1000,init.passive=7000)
  {
	#  AUTHOR: 
  ...
  #=========================================================================================
	...
	for(year in years){
	  ...
	  cinput     <- SiteData$cinput[SiteData$year == year]
		...
		print('numerador:')
		numerador <- ((Beta * f1) + ((cinput * (1 - L) - Beta) * f2) + (cinput * L * f3 * (f7 + f6 * f8)))
		print(numerador)
		print('divisor:')
		divisor <- (1 - f4 * f7 - f5 * f8 - f4 * f6 * f8)
		print(divisor)
		#=========================================================================================
	...
	write.csv(SOMstocks,"./cio2.csv", row.names = TRUE)
	return(SOMstocks)
}
